...
## 4. Methodology
ALM v12.0 implements a modular pipeline:
1. **Perception**: Whisper Large-v3 and CLAP/HTS-AT.
2. **Auditory World Model & PSE**: Segregating primary vs background audio.
3. **Evidence Fusion**: Merging facts into an `AudioEvidenceObject`.
4. **Semantic Interpretation**: A prompted Qwen2.5-3B-Instruct model that outputs strictly formatted JSON.
5. **Cognitive State Management**: A deterministic engine consisting of HRE, WSE, SPE, and TRE that maintains a temporal memory of the auditory scene.
6. **Intelligence Rendering**: Generates human-readable output (Speech, Environment, Situation).
...
- **Uncertainty Handling:** 4.64 (ALM correctly demotes low-confidence scenes instead of hallucinating).
...
## 8. Limitations
The requirement to load Whisper, CLAP, and Qwen 3B concurrently demands significant local RAM (~5-8GB), which limits deployment on highly edge-constrained devices. Furthermore, when Whisper's VAD fails in high-noise environments, the Semantic Layer is starved of linguistic context.
