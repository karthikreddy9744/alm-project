# ALM Documentation Index

Welcome to the ALM Documentation directory. This folder serves as the single source of truth for the ALM v12.0 Structured Reasoning Architecture, research philosophies, commands, and developer practices. 

## 📂 Documentation Files

### 1. [ALM_MiniProject.md](./ALM_MiniProject.md)
**Purpose:** The definitive technical documentation for the frozen ALM v12.0 architecture. This is the single source of truth for the entire project.
**Contents:**
- Problem Statement & Vision
- Full Cognitive Pipeline Flow (Perception -> Speech -> Observation -> Provenance -> Hypothesis -> Verification -> Influence -> Interpretation)
- Detailed Module Responsibilities (Whisper, CLAP, HRE, WSE, SPE, TRE, SIR)
- Human-Oriented Auditory Situation Understanding
- Pydantic Schema and Prompt Engineering Philosophies
- Future Research and Glossary

### 2. [COMMANDS.md](./COMMANDS.md)
**Purpose:** The detailed, expanded manual for all CLI interactions.
**Contents:**
- Commands for executing the end-to-end pipeline.

## 🚀 Navigation Quick Links
- **Want to run the app?** See `COMMANDS.md` or the `../COMMANDS` cheat sheet.
- **Want to understand the cognitive pipeline?** Read `ALM_MiniProject.md`.

## ⚠️ Notes for Future Contributors
The ALM Structured Reasoning Architecture is **permanently frozen** around the Human-Oriented Auditory Situation Understanding paradigm. Future research efforts should focus on *Temporal Memory Scaling* and *LLM Distillation*, rather than re-introducing stochastic LLM generation into the core reasoning loop.
